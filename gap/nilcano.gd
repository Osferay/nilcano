...
#! This package also includes functions to compute the intersection of subgroups of nilpotent
#! groups and to compute the product of subgroups of nilpotent groups.
	
#! @Section Nilpotent sequences
#! Let $G$ be a finitely generated nilpotent group. Then $G$ has a series $G = G_1 > G_2 > \dots > G_n > G_{n+1} = \{1\}$
#! so that each $G_i$ is normal in $G$ and each quotient $G_i/G_{i+1}$ is cyclic and central in $G/G_{i+1}$. 
#! We call such a series a nilpotent series of $G$. Then $(g_1,\dots, g_n)$ is called a nilpotent sequence
#! of $G$ and $(o_1,\dots, o_n)$ are its relative orders. Let $I=\{ i\in \mathbb{Z} \mid o_i \neq \infty \}$.
#! Note that the nilpotent sequence determines its
#! nilpotent series by $G_i = \langle g_i,\dots, g_n \rangle$ holds for $1 \leq i \leq n$.
#!
...
#! For $g \in G$ and $U \leq G$, we denote the conjugacy class of $g$ under $U$ by $g^U = \{g^h \mid h \in U\}$
#! and the centralizer of $g$ in $U$ by $C_U(g) = \{h \in U \mid g^h = g\}$.
#!
#! Let $\ll$ be the well-order $0 \ll 1 \ll 2 \ll \dots \ll −1 \ll −2 \dots$ of $\mathbb{Z}$.
#! We extend the well-order to exponent vectors lexicographically; that is, $(e_1,\dots, e_n) \ll (f_1, \dots, f_n)$
...
#! Suppose that the finitely generated group $G$ is given by a nilpotent sequence $(g_1,\dots,g_n)$. 
#! Given $U, V \leq G$ we write $U^V = \{U^g \mid g ∈ V \}$ for the conjugacy class of $U$ under $V$ 
...
#! @Description
#! Given a list of elements of a nilpotent group $G$, returns a list of canonical conjugacy representatives of the list and the positions of the elements that
#! belong to the given canonical conjugacy class.
#! @Arguments G, list
DeclareGlobalFunction( "CanonicalConjugateList" );
#! @Description
#! Given a list of elements of a nilpotent group $G$, returns a list of conjugacy representatives of the list and the positions of the elements that
...
#! These algorithms are based on Eddie's work <Cite Key="Eddie" />.
...
#! @Description
#! Given two subgroups $U,V$ of a nilpotent group $G$ and an element $g$ in $G$ computes the decomposition of $g$
#! with respect to the product pair of the two subgroups.
#! @Arguments G, U, V, g
